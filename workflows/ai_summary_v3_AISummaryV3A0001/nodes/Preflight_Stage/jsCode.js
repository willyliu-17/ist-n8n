...
if (done) return [];
return [{ json: { ...carrier, row, ownerConditions: { id: row.id, requestKey: row.requestKey, status: row.status, reconciliationStatus: 'canonical', canonicalRowID: row.id, leaseOwner: row.leaseOwner, leaseUntilIso: row.leaseUntilIso } } }];
