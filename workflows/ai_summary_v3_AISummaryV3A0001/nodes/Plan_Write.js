function currentOwner(carrier, rows) {
  ...
  const canonical = rows.filter((item) => item && item.reconciliationStatus === 'canonical' && item.canonicalRowID === item.id);
...
function planWrite(items) {
  ...
  if (!values || Object.values(values).some((value) => value === undefined || value === '')) throw new Error('checkpoint payload missing');
