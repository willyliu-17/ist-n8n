...
if (canonical.length !== 1) {
  ...
  return targets.map((row) => ({ json: {
    ...
    values: action === 'freeze'
      ...
      : { reconciliationStatus: 'duplicate', canonicalRowID: winner.id },
  } }));
}
...
return [{ json: { kind: 'carrier', action: stage, input: input.input, requestKey: input.input.requestKey, row, ownerConditions: { id: row.id, requestKey: row.requestKey, status: row.status, reconciliationStatus: 'canonical', canonicalRowID: row.id, leaseOwner: row.leaseOwner, leaseUntilIso: row.leaseUntilIso }, nextStage: stage } }];
