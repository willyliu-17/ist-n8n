...
function planWrite(items) {
  ...
  const values = carrier.nextStage === 'inference' ? { inferenceResultJson: carrier.inferenceResultJson, summaryMarkdown: carrier.summaryMarkdown } : carrier.nextStage === 'upload' ? { summaryUploadID: carrier.checkpointValue } : carrier.nextStage === 'status' ? { summaryMessageTS: carrier.checkpointValue } : carrier.nextStage === 'complete' ? { status: 'completed', nextRetryAtIso: '', errorCode: '', leaseOwner: '', leaseUntilIso: '' } : null;
