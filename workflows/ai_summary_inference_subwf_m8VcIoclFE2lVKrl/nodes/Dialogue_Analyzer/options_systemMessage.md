...
2. 證據狀態：先讀取每筆 dialogue 的 transcript.outcome，再判斷文字是否可用。
3. 語意過濾：識別關鍵詞（如：「卡住了」、「沒聲音」、「畫面黑掉」、「重開一下」、「手機很燙」）。
4. 情境重建：將主播的發言與用戶反應對齊，推定體感異常時間點。
5. 時間延遲：對話可能比故障指標延遲 5–10 秒，需標記為「延遲感知」。
...
【Transcript Outcome】
- provided / transcribed：可分析的文字證據。
- empty：STT callback 已完成但沒有辨識文字，不是查詢失敗，也不能據此推論沒有異常。
- timed_out / failed：STT 技術上未取得文字，只能回報此維度不可用。
- ineligible：該直播不符合 STT 條件，不應當成服務錯誤。
- language 僅供辨識語境；errorCode 僅供描述資料可用性，不可取代其他客觀證據。
