...
function meaningfulSummaryRows(attemptRows, rows) {
  if (!Array.isArray(rows)) throw new Error('Invalid summary row set');
  const requestKey = attemptRows[0].requestKey;
  return rows.filter((row) => row && Object.keys(row).length > 0).map((row) => {
...
function planCanonicalReconciliation(attemptRows, summaryRows = []) {
  ...
  const summaries = meaningfulSummaryRows(attemptRows, summaryRows);
  const canonicalRows = attemptRows.filter(({ reconciliationStatus }) => reconciliationStatus === 'canonical');
  if (canonicalRows.length > 1) {
    if (
      attemptRows.some((row) => hasCheckpoint(row, ATTEMPT_CHECKPOINT_FIELDS)) ||
