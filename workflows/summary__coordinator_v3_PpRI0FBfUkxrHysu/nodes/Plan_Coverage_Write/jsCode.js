...
function planCoverageWrite(request, aggregate) {
  ...
  if (request.status === 'waiting_stt') return { action: 'write_ready', id: request.id, requestKey: request.requestKey, expected: { status: 'waiting_stt', reconciliationStatus: 'canonical', canonicalRowID: String(request.id), coverageStatus: request.coverageStatus, availableRolesJson: request.availableRolesJson, missingRolesJson: request.missingRolesJson, failedLogicalJobKeysJson: request.failedLogicalJobKeysJson }, desired: { status: 'ready', ...snapshot, leaseOwner: '', leaseUntilIso: '' } };
  if (['ready', 'summary_dispatching', 'summary_retry_pending'].includes(request.status)) {
    for (const [field, value] of Object.entries(snapshot)) if (request[field] !== value) throw new Error(`coverage replay mismatch: ${field}`);
    return { action: 'replay', id: request.id, requestKey: request.requestKey, expected: { status: request.status, ...snapshot } };
  }
  return { action: 'noop', reason: 'status_not_coverage_writable', status: request.status };
}
function planCoverageFromRuntime(aggregate) {
  if (!aggregate?.persistedRequest) throw new Error('coverage runtime requires persisted request');
  return planCoverageWrite(aggregate.persistedRequest, aggregate);
}
function planAllFailedWrite(request) {
  if (!request?.id || request.reconciliationStatus !== 'canonical' || request.canonicalRowID !== String(request.id)) throw new Error('all-failed requires canonical request');
  ...
  return { action: 'all_failed', id: request.id, requestKey: request.requestKey, expected: { status: request.status, reconciliationStatus: 'canonical', canonicalRowID: String(request.id), leaseOwner: request.leaseOwner || '', leaseUntilIso: request.leaseUntilIso || '' }, desired };
}
function verifyCoverageWrite(rows, plan) {
  const canonical = rows.filter((row) => row?.requestKey === plan.requestKey && row.reconciliationStatus === 'canonical' && row.canonicalRowID === String(row.id));
...
if (typeof module !== 'undefined' && module.exports) module.exports = { coverageSnapshot, planAllFailedWrite, planCoverageFromRuntime, planCoverageWrite, verifyCoverageWrite };
if (typeof $input !== 'undefined') {
  return [{ json: planCoverageFromRuntime($input.first().json) }];
}
