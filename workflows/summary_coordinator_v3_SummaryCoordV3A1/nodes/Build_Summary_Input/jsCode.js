function buildSummaryInput(request, aggregate) {
  ...
  return { requestKey: request.requestKey, requestType: request.requestType, channel: request.channel, threadTS: request.threadTS, coverageStatus: aggregate.coverageStatus, availableRoles: aggregate.availableRoles, missingRoles: aggregate.missingRoles, failedLogicalJobKeys: aggregate.failedLogicalJobKeys, streams: aggregate.streams.map(({ role, liveStreamID, mode, dialogue, streamContext }) => ({ role, liveStreamID, mode, dialogue, streamContext })) };
}
