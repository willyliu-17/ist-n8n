function splitPlanAndRows(items, phase) {
  ...
  if (plans.some((plan) => plan.__planPhase !== phase)) throw new Error('mixed phase plan carrier');
  if (plans.some((plan) => plan.__planAction !== plan.action)) throw new Error('mixed action plan carrier');
  const ids = new Set();
...
if (typeof $input !== 'undefined') {
  return verifyCarrierInput($input.all().map(({ json }) => ({ ...json })), 'initial').map((json) => ({ json }));
}
